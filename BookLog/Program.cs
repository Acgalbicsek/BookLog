...
namespace BookLog
{
    ...
    {
        // Dictionary of user → BookLedger
        private readonly Dictionary<string, BookLedger> _ledgers = new(StringComparer.OrdinalIgnoreCase);

        // Get or create a ledger for a user
        public BookLedger GetLedger(string userName)
        {
            if (string.IsNullOrWhiteSpace(userName))
                throw new ArgumentException("User name cannot be blank.");

            if (!_ledgers.TryGetValue(userName, out var ledger))
            {
                ledger = new BookLedger();
                _ledgers[userName] = ledger;
            }
        ...
        public Dictionary<string, BookLedger> GetAllLedgers() => _ledgers;
        public void LoadAll(Dictionary<string, BookLedger> data)
        {
            _ledgers.Clear();
            foreach (var kvp in data)
                _ledgers[kvp.Key] = kvp.Value;
        }
    }
    ...
    internal static class Program
    {
        ...
        private static readonly string ExportTxt =
    Path.Combine(AppContext.BaseDirectory, "BookLog_Export.txt");

        private static readonly string DataFile = Path.Combine(AppContext.BaseDirectory, "booklog.json");
        private static readonly LedgerManager _manager = new();
        private static BookLedger _ledger; // currently active ledger
        ...
        private static void Main()
        {
            ...
            _currentUser = Console.ReadLine()?.Trim() ?? "Unknown";

            _ledger = _manager.GetLedger(_currentUser);
        ...
        private static void RunMenu()
        {
            ...
            {
                Console.Clear();
                Console.WriteLine($"=== BOOK LOG for {_currentUser} ===");
                Console.WriteLine("1) Add entry");
                Console.WriteLine("2) List entries");
                Console.WriteLine("3) Show totals & amount owed");
                Console.WriteLine("4) Export to Notepad (.txt)");
                ...
                switch (choice)
                {
                    ...
                    case "4": ExportToTxtAndOpen(); break;
                    case "5": MarkPaid(); break;
                    case "6": ResetAll(); break;
                    case "7": SwitchUser(); break;
                    case "8":
                        Save();
        ...
        private static void SwitchUser()
        {
            ...
            _currentUser = name;
            _ledger = _manager.GetLedger(name);

            Console.WriteLine($"Switched to {_currentUser}. Press any key...");
            Console.ReadKey();
        }
        ...
        {
            var data = _manager.GetAllLedgers();
        ...
        private static void Load()
        {
            try
            {
                if (File.Exists(DataFile))
                {
                    ...
                    var loaded = JsonSerializer.Deserialize<Dictionary<string, BookLedger>>(json, new JsonSerializerOptions
                    {
                        PropertyNameCaseInsensitive = true
                    });

                    if (loaded != null)
                        _manager.LoadAll(loaded);
                }
            }
            catch
            {
                // fallback to blank manager if file corrupt
            }
        }
